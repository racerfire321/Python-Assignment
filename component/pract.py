...
class bookingDashboard:
    ...
    def add_path_between_points(self, origin, destination):
        ...
        if origin_coords and destination_coords:
            ...
            map_center = [(origin_coords[0] + destination_coords[0]) / 2,
                          (origin_coords[1] + destination_coords[1]) / 2]
            my_map = folium.Map(location=map_center, zoom_start=10)

            # Add markers for the two points
            folium.Marker(location=origin_coords, popup='Point A').add_to(my_map)
            folium.Marker(location=destination_coords, popup='Point B').add_to(my_map)

            # Add a line between the two points
            folium.PolyLine(locations=[origin_coords, destination_coords], color='blue').add_to(my_map)

            # Save the map to an HTML file
            my_map.save("map.html")

            # Open the HTML file using a WebView
            webview.create_window("Map", url="map.html", width=800, height=600)
            webview.start()
        else:
